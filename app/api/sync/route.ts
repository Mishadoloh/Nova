...
type SyncPayload = {
  ...
  preferences?: { focusMinutes: number; breakMinutes: number; autoPomodoro: boolean; dailyGoalMinutes?: number };
  baseRevision?: number;
  force?: boolean;
};
...
async function readUserData(userId: string) {
  ...
  const [projectRows,taskRows,sessionRows,eventRows,preferenceRows,metaRows]=await db.batch([
    ...
    db.prepare("SELECT focus_minutes AS focusMinutes,break_minutes AS breakMinutes,auto_pomodoro AS autoPomodoro,daily_goal_minutes AS dailyGoalMinutes FROM user_preferences WHERE user_id=?").bind(userId),
...
export async function POST(request: Request) {
  ...
  const preferences=payload.preferences??{focusMinutes:25,breakMinutes:5,autoPomodoro:false,dailyGoalMinutes:120}; const timestamp=Date.now(); const nextRevision=currentRevision+1;
  ...
  statements.push(db.prepare("INSERT INTO user_preferences (user_id,focus_minutes,break_minutes,auto_pomodoro,daily_goal_minutes,updated_at) VALUES (?,?,?,?,?,?) ON CONFLICT(user_id) DO UPDATE SET focus_minutes=excluded.focus_minutes,break_minutes=excluded.break_minutes,auto_pomodoro=excluded.auto_pomodoro,daily_goal_minutes=excluded.daily_goal_minutes,updated_at=excluded.updated_at").bind(user.userId,Math.min(120,Math.max(1,Number(preferences.focusMinutes))),Math.min(60,Math.max(1,Number(preferences.breakMinutes))),preferences.autoPomodoro?1:0,Math.min(720,Math.max(15,Number(preferences.dailyGoalMinutes??120))),timestamp));
