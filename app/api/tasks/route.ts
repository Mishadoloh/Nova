import { apiContext, apiError, apiOk, bumpRevision, cleanId, cleanNumber, cleanText, jsonBody, unauthorized } from "../backend";
...
export async function GET(request:Request) {
  const context=await apiContext();if(!context)return unauthorized();const url=new URL(request.url),projectId=cleanId(url.searchParams.get("projectId")),selected=status(url.searchParams.get("status"));let query="SELECT id,project_id AS projectId,text,done,status,deadline,recurrence,sort_order AS sortOrder,created_at AS createdAt,updated_at AS updatedAt FROM tasks WHERE user_id=?",values:unknown[]=[context.user.userId];
