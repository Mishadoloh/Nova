...
export function RegistrationForm() {
  ...
  const passwordStrength = [password.length >= 8, /[A-Za-zА-Яа-яІіЇїЄє]/.test(password), /\d/.test(password)].filter(Boolean).length;
  ...
  if (mode === "password") return <main className="registration-page"><section className="registration-shell registration-signin"><a className="registration-brand" href="/"><span>N</span>NOVA</a><span className="eyebrow">Відновлення доступу</span><h1>Створи новий<br/><em>пароль.</em></h1><p>Використай щонайменше 8 символів. Після збереження NOVA автоматично відкриє твій простір.</p><form className="password-update-form" onSubmit={updatePassword}><label className="auth-password-field"><span>Новий пароль</span><input type={showPassword?"text":"password"} value={password} onChange={(event)=>setPassword(event.target.value)} minLength={8} required autoComplete="new-password" placeholder="Мінімум 8 символів"/><button type="button" onClick={()=>setShowPassword((visible)=>!visible)} aria-label={showPassword?"Приховати пароль":"Показати пароль"} aria-pressed={showPassword}>{showPassword?"Сховати":"Показати"}</button></label><div className="password-strength" data-score={passwordStrength} aria-live="polite"><i/><i/><i/><span>{passwordStrength<2?"Додай літери й цифри":passwordStrength===2?"Надійний пароль":"Сильний пароль"}</span></div>{message&&<p className={success?"registration-success":"registration-error"}>{message}</p>}<button className="registration-submit" disabled={saving}>{saving?"Зберігаю…":"Зберегти пароль →"}</button></form></section></main>;
