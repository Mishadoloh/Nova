...
const initialLevels: Levels = { rain: 42, cafe: 24, forest: 30, wind: 22 };
const presets = [
  { id: "deep", name: "Deep work", icon: "◎", levels: { rain: 46, cafe: 12, forest: 0, wind: 0 } },
  { id: "nature", name: "Природа", icon: "◇", levels: { rain: 12, cafe: 0, forest: 44, wind: 14 } },
  { id: "storm", name: "Шторм", icon: "↯", levels: { rain: 62, cafe: 0, forest: 0, wind: 38 } },
  { id: "coffee", name: "Кавова пауза", icon: "◉", levels: { rain: 12, cafe: 52, forest: 0, wind: 0 } },
];
...
export function AudioMixer() {
  ...
  useEffect(() => {
    const cleanLevels = Object.fromEntries(tracks.map((track) => [track.id, levels[track.id] ?? initialLevels[track.id]]));
    localStorage.setItem("nova-audio-mixer-v2", JSON.stringify({ levels: cleanLevels, master }));
  ...
  const applyPreset = (selected: typeof presets[number]) => {
    ...
    tracks.forEach((track) => {
      const level = selected.levels[track.id as keyof typeof selected.levels] ?? 0;
      if (level > 0) {
        const element = getAudio(track);
        element.volume = level / 100 * master / 100;
        element.play().catch(() => setFailed(track.id));
        next[track.id] = true;
      }
    });
    setActive(next);
