"use client";

import { useEffect, useMemo, useState } from "react";
import { navItems } from "./AppFrame";
import { useNovaStore } from "./nova-store";

type CommandResult = {
  id?: string;
  label: string;
  href: string;
  meta: string;
  color?: string | null;
};

type ServerSearchResult = {
  id: string;
  type: "project" | "task" | "event";
  title: string;
  subtitle: string;
  href: string;
  color: string | null;
};

const resultLabels = {
  project: "Проєкт",
  task: "Задача",
  event: "Подія календаря",
} as const;

function navigateTo(href: string) {
  window.location.assign(href);
}

export function GlobalTools() {
  const { data, save, ready } = useNovaStore();
  const [open, setOpen] = useState(false);
  const [query, setQuery] = useState("");
  const [activeResult, setActiveResult] = useState(0);
  const [onboarding, setOnboarding] = useState(false);
  const [step, setStep] = useState(0);
  const [goal, setGoal] = useState("Робота");
  const [minutes, setMinutes] = useState(25);
  const [project, setProject] = useState("");
  const [serverResults, setServerResults] = useState<CommandResult[]>([]);
  const [searchState, setSearchState] = useState<
    "idle" | "loading" | "ready" | "error"
  >("idle");
  useEffect(() => {
    const key = (event: KeyboardEvent) => {
      const target = event.target as HTMLElement;
      const typing =
        target.isContentEditable ||
        ["INPUT", "TEXTAREA", "SELECT"].includes(target.tagName);
      if ((event.ctrlKey || event.metaKey) && event.key.toLowerCase() === "k") {
        event.preventDefault();
        setOpen((value) => !value);
      }
      if (event.key === "Escape") setOpen(false);
      if (!typing && !event.ctrlKey && !event.metaKey) {
        if (event.key === "/") {
          event.preventDefault();
          setOpen(true);
        }
        if (event.key.toLowerCase() === "f") navigateTo("/");
        if (event.key.toLowerCase() === "p") navigateTo("/projects");
        if (event.key.toLowerCase() === "c") navigateTo("/calendar");
      }
    };
    window.addEventListener("keydown", key);
    return () => window.removeEventListener("keydown", key);
  }, []);
  useEffect(() => {
    const show = () => setOpen(true);
    window.addEventListener("nova-open-command", show);
    return () => window.removeEventListener("nova-open-command", show);
  }, []);
  useEffect(() => {
    if (ready)
      setOnboarding(
        data.projects.length === 0 && !localStorage.getItem("nova-onboarded"),
      );
  }, [ready, data.projects.length]);
  useEffect(() => {
    if ("serviceWorker" in navigator)
      navigator.serviceWorker.register("/sw.js").catch(() => undefined);
  }, []);

  useEffect(() => {
    const needle = query.trim();
    if (!open || needle.length < 2) {
      setServerResults([]);
      setSearchState("idle");
      return;
    }
    const controller = new AbortController();
    const timeout = window.setTimeout(async () => {
      setSearchState("loading");
      try {
        const response = await fetch(
          `/api/search?q=${encodeURIComponent(needle)}&limit=8`,
          { signal: controller.signal, cache: "no-store" },
        );
        if (!response.ok) throw new Error("search");
        const payload = (await response.json()) as {
          data?: { results?: ServerSearchResult[] };
        };
        setServerResults(
          (payload.data?.results ?? []).map((item) => ({
            id: item.id,
            label: item.title,
            href: item.href,
            meta: `${resultLabels[item.type]} · ${item.subtitle}`,
            color: item.color,
          })),
        );
        setSearchState("ready");
      } catch (error) {
        if ((error as Error).name !== "AbortError") setSearchState("error");
      }
    }, 220);
    return () => {
      window.clearTimeout(timeout);
      controller.abort();
    };
  }, [open, query]);

  const results = useMemo(() => {
    const needle = query.trim().toLocaleLowerCase("uk-UA");
    const navigation: CommandResult[] = navItems
      .map((item) => ({
        label: item[2],
        href: item[3],
        meta: "Сторінка",
      }))
      .filter((item) => item.label.toLocaleLowerCase("uk-UA").includes(needle));
    const localTasks: CommandResult[] = data.tasks
      .map((task) => ({
        id: task.id,
        label: task.text,
        href: `/projects?task=${encodeURIComponent(task.id)}`,
        meta: "Задача",
        color:
          data.projects.find((item) => item.id === task.projectId)?.color ??
          null,
      }))
      .filter((item) => item.label.toLocaleLowerCase("uk-UA").includes(needle));
    const content =
      needle.length >= 2 && searchState === "ready"
        ? serverResults
        : localTasks;
    const seen = new Set<string>();
    return [...navigation, ...content]
      .filter((item) => {
        const key = `${item.href}:${item.id ?? item.label}`;
        if (seen.has(key)) return false;
        seen.add(key);
        return true;
      })
      .slice(0, 9);
  }, [query, data.tasks, data.projects, serverResults, searchState]);
  const canQuickCreate = Boolean(query.trim() && data.projects.length > 0);
  const commandCount = results.length + (canQuickCreate ? 1 : 0);

  useEffect(() => {
    setActiveResult(0);
  }, [query, open]);
  const finish = async () => {
    const timestamp = Date.now();
    const first = {
      id: `project-${timestamp}`,
      name: project.trim() || goal,
      color: "#dfff00",
      createdAt: timestamp,
      updatedAt: timestamp,
    };
    const next = {
      ...data,
      projects: [first],
      tasks: [],
      sessions: [],
      events: [],
      preferences: { ...data.preferences, focusMinutes: minutes },
    };
    await save(next);
    localStorage.setItem("nova-onboarded", "1");
    setOnboarding(false);
    navigateTo("/");
  };
  const quickAdd = () => {
    const text = query.trim(),
      projectId =
        data.projects.find(
          (item) =>
            item.id === data.preferences.activeProjectId && !item.archived,
        )?.id ?? data.projects.find((item) => !item.archived)?.id;
    if (!text || !projectId) return;
    const stamp = Date.now();
    save({
      ...data,
      tasks: [
        ...data.tasks,
        {
          id: `task-${stamp}`,
          projectId,
          text,
          done: false,
          status: "todo",
          createdAt: stamp,
          updatedAt: stamp,
        },
      ],
    }).finally(() => {
      setQuery("");
      setOpen(false);
      navigateTo("/projects");
    });
  };
  const runCommand = (index: number) => {
    if (index < results.length) {
      setQuery("");
      setOpen(false);
      navigateTo(results[index].href);
      return;
    }
    if (canQuickCreate) quickAdd();
  };
  return (
    <>
      {
        <button
          className="command-trigger"
          type="button"
          onClick={() => setOpen(true)}
          aria-label="Відкрити командну панель"
        >
          ⌘ <span>Ctrl K</span>
        </button>
      }
      {open && (
        <div
          className="command-backdrop"
          role="presentation"
          onMouseDown={() => setOpen(false)}
        >
          <section
            className="command-panel"
            role="dialog"
            aria-modal="true"
            aria-label="Командна панель"
            onMouseDown={(event) => event.stopPropagation()}
          >
            <div className="command-search">
              <span>⌕</span>
              <input
                autoFocus
                value={query}
                onChange={(event) => setQuery(event.target.value)}
                onKeyDown={(event) => {
                  if (!commandCount) return;
                  if (event.key === "ArrowDown") {
                    event.preventDefault();
                    setActiveResult((value) => (value + 1) % commandCount);
                  }
                  if (event.key === "ArrowUp") {
                    event.preventDefault();
                    setActiveResult(
                      (value) => (value - 1 + commandCount) % commandCount,
                    );
                  }
                  if (event.key === "Enter") {
                    event.preventDefault();
                    runCommand(activeResult);
                  }
                }}
                placeholder="Шукати або швидко додати задачу…"
                aria-label="Пошук"
                role="combobox"
                aria-expanded="true"
                aria-controls="nova-command-results"
                aria-activedescendant={
                  commandCount ? `nova-command-${activeResult}` : undefined
                }
              />
              <kbd>ESC</kbd>
            </div>
            <div
              className="command-results"
              id="nova-command-results"
              role="listbox"
              aria-label="Результати пошуку"
            >
              <span className="eyebrow">Швидкий перехід</span>
              {searchState === "loading" && (
                <div className="command-status" role="status">
                  <i /> Пошук у хмарних даних…
                </div>
              )}
              {searchState === "error" && (
                <div className="command-status warning" role="status">
                  Показано локальні результати
                </div>
              )}
              {results.map((item, index) => (
                <button
                  id={`nova-command-${index}`}
                  key={`${item.href}-${index}`}
                  type="button"
                  role="option"
                  aria-selected={activeResult === index}
                  className={activeResult === index ? "active" : ""}
                  onMouseEnter={() => setActiveResult(index)}
                  onClick={() => runCommand(index)}
                >
                  <i
                    className={item.color ? "result-color" : ""}
                    style={
                      item.color
                        ? ({ "--result-color": item.color } as React.CSSProperties)
                        : undefined
                    }
                  >
                    {item.color ? "" : String(index + 1).padStart(2, "0")}
                  </i>
                  <span>
                    {item.label}
                    <small>{item.meta}</small>
                  </span>
                  <b>↗</b>
                </button>
              ))}
              {canQuickCreate && (
                <button
                  id={`nova-command-${results.length}`}
                  className={`quick-create ${activeResult === results.length ? "active" : ""}`}
                  type="button"
                  role="option"
                  aria-selected={activeResult === results.length}
                  onMouseEnter={() => setActiveResult(results.length)}
                  onClick={() => runCommand(results.length)}
                >
                  <i>+</i>
                  <span>
                    Створити задачу «{query.trim()}»
                    <small>
                      {data.projects.find((item) => !item.archived)?.name}
                    </small>
                  </span>
                  <b>↵</b>
                </button>
              )}
              {!commandCount && (
                <div className="command-empty">
                  <i>⌕</i>
                  <span>
                    Нічого не знайдено
                    <small>Спробуй інший запит</small>
                  </span>
                </div>
              )}
            </div>
            <footer>
              <span>↑↓ вибрати</span>
              <span>Enter відкрити</span>
              <span>ESC закрити</span>
              <span>Ctrl K будь-де</span>
            </footer>
          </section>
        </div>
      )}
      {onboarding && (
        <div className="onboarding-backdrop">
          <section
            className="onboarding-card"
            role="dialog"
            aria-modal="true"
            aria-label="Знайомство з NOVA"
          >
            <div className="onboarding-progress">
              {[0, 1, 2].map((item) => (
                <i className={item <= step ? "active" : ""} key={item} />
              ))}
            </div>
            {step === 0 && (
              <>
                <span className="onboarding-mark">N</span>
                <span className="eyebrow">Ласкаво просимо</span>
                <h2>
                  Знайди свій
                  <br />
                  <em>ритм.</em>
                </h2>
                <p>NOVA допоможе зберегти увагу на тому, що справді важливо.</p>
                <button onClick={() => setStep(1)}>
                  Налаштувати простір →
                </button>
              </>
            )}
            {step === 1 && (
              <>
                <span className="eyebrow">Крок 1 із 2</span>
                <h2>
                  Що зараз
                  <br />
                  найважливіше?
                </h2>
                <div className="goal-options">
                  {["Робота", "Навчання", "Особистий розвиток"].map((item) => (
                    <button
                      className={goal === item ? "active" : ""}
                      onClick={() => setGoal(item)}
                      key={item}
                    >
                      {item}
                    </button>
                  ))}
                </div>
                <label className="rhythm-select">
                  Тривалість фокусу <strong>{minutes} хв</strong>
                  <input
                    type="range"
                    min="10"
                    max="60"
                    step="5"
                    value={minutes}
                    onChange={(event) => setMinutes(Number(event.target.value))}
                  />
                </label>
                <button onClick={() => setStep(2)}>Продовжити →</button>
              </>
            )}
            {step === 2 && (
              <>
                <span className="eyebrow">Крок 2 із 2</span>
                <h2>
                  Назви перший
                  <br />
                  проєкт.
                </h2>
                <input
                  className="project-name-input"
                  autoFocus
                  value={project}
                  onChange={(event) => setProject(event.target.value)}
                  placeholder={goal}
                />
                <p>
                  Порожній простір без демонстраційних задач — лише твої
                  справжні цілі.
                </p>
                <button onClick={finish}>Увійти в NOVA →</button>
              </>
            )}
          </section>
        </div>
      )}
    </>
  );
}
